...
class data():
    def __init__(self,text:str,json:dict):
        self.text=text
        self.json=json
    ...
    async def main():
        async with ClientSession(headers=headers,timeout=ClientTimeout(None))as client:
            async with client.request(method,url,data=body,proxy=proxy)as res:
                result=data(await res.text(),await res.json())
        return result
...
def 单线程爬数据get(url:str,header:str,handle:function,var:list,proxy:str=None)->list:
    headers={}
    for h in header.splitlines():
        x=h.split(r': ')
        headers[x[0]]=x[1]
...
def 单线程爬数据post(url:str,header:str,body:str,handle:function,var:list,proxy:str=None)->list:
    headers={}
    for h in header.splitlines():
        x=h.split(r': ')
        headers[x[0]]=x[1]
